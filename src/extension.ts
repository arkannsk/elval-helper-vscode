...
import { handleEvlAnnotations } from './handlers/evlHandler';
import { handleOaAnnotations } from './handlers/oaHandler';
import { getDecorations } from './decorators';

// Глобальная переменная для хранения последних примененных декораций (для очистки)
let currentDecorations: ReturnType<typeof getDecorations> | null = null;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    function updateAllDecorations(forceRefresh = false) {
        ...
        const allDirectives: vscode.DecorationOptions[] = [];
        const allSimpleParams: vscode.DecorationOptions[] = [];
        const allKvParams: vscode.DecorationOptions[] = [];
        const allValues: vscode.DecorationOptions[] = [];
        const allOaKeys: vscode.DecorationOptions[] = [];
        const allLocations: vscode.DecorationOptions[] = [];
        const allParamNames: vscode.DecorationOptions[] = [];
        ...
        lines.forEach((line, lineIndex) => {
            ...
            const evlResult = {
                directives: [] as vscode.DecorationOptions[],
                simpleParams: [] as vscode.DecorationOptions[],
                kvParams: [] as vscode.DecorationOptions[],
                values: [] as vscode.DecorationOptions[]
            };

            const oaResult = {
                oaKeys: [] as vscode.DecorationOptions[],
                values: [] as vscode.DecorationOptions[],
                locations: [] as vscode.DecorationOptions[],
                paramNames: [] as vscode.DecorationOptions[]
            };

            handleEvlAnnotations(fullCommentText, lineIndex, baseOffset, evlResult, line);
            handleOaAnnotations(fullCommentText, lineIndex, baseOffset, oaResult, line);
    ...
    // Команда смены цвета
        ...
        const selected = await vscode.window.showQuickPick(
            settingsList.map(s => ({ 
                label: s.label, 
                description: `Current: ${config.get<any>(s.key)}`, 
                key: s.key,
                type: s.type,
                options: s.options
            })),
        ...
        if (selected.type === 'enum' && selected.options) {
            ...
            if (picked) newValue = picked.label;
        } else {
            // Ввод цвета (HEX)
            const currentVal = config.get<string>(selected.key);
